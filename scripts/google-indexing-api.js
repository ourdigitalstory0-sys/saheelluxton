...
function pingGoogleSitemap() {
  ['https://saheeluxton.in/sitemap.xml', 'https://www.saheeluxton.in/sitemap.xml'].forEach(sm => {
    ...
    https.get(pingUrl, (res) => {
      console.log(`[Google Ping] Response for ${sm}: ${res.statusCode} (${res.statusMessage})`);
    }).on('error', (e) => {
      console.log(`[Google Ping] Network response recorded for ${sm}: ${e.message}`);
    });
  });
}
...
function pingBingSitemap() {
  ['https://saheeluxton.in/sitemap.xml', 'https://www.saheeluxton.in/sitemap.xml'].forEach(sm => {
    ...
    https.get(pingUrl, (res) => {
      console.log(`[Bing Ping] Response for ${sm}: ${res.statusCode} (${res.statusMessage})`);
    }).on('error', (e) => {
      console.log(`[Bing Ping] Network response recorded for ${sm}: ${e.message}`);
    });
  });
}
