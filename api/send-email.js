...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Gmail SMTP credentials from Vercel Environment Variables
    const gmailUser = process.env.GMAIL_USER || 'propsmartrealty@gmail.com';
    const gmailAppPassword = (process.env.GMAIL_APP_PASSWORD || process.env.GOOGLE_APP_PASSWORD || '').replace(/\s+/g, '');

    if (!gmailAppPassword) {
      console.warn('⚠️ GMAIL_APP_PASSWORD is not set in Vercel environment variables.');
      return res.status(500).json({
        success: false,
        message: 'GMAIL_APP_PASSWORD is not set in Vercel. Please add it under Vercel Project Settings > Environment Variables.'
      });
    }
